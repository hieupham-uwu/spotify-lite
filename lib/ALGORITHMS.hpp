...
/*
  Usage:
    1. Sort the ENTIRE vector:
        ...
        - Custom criteria (Using Lambda function for custom structures/objects):
            mergeSort(myVector, [](const MyStruct& a, const MyStruct& b) { return a.value > b.value; });
  ...
  All functions:
+ Sorting (Range-based & Full): bubbleSort(), selectionSort(), insertionSort(), heapSort(), quickSort(), mergeSort()
...
template <typename T, typename Comp = std::less<T>>
void selectionSort(std::vector<T>& vec, int left, int right, Comp cmp = Comp{}) {
    if (left >= right || left < 0 || right >= (int)vec.size()) return;
...
template <typename T, typename Comp = std::less<T>>
void insertionSort(std::vector<T>& vec, int left, int right, Comp cmp = Comp{}) {
    if (left >= right || left < 0 || right >= (int)vec.size()) return;
